// ================= src/components/home/ShoppingMode.jsx =================
import { Wallet, Crown, Flame } from "lucide-react";

function ShoppingMode({ shoppingMode, setShoppingMode }) {
  const modes = [
    { name: "HEMAT", desc: "Harga terbaik", icon: <Wallet size={26} /> },
    { name: "PREMIUM", desc: "Produk pilihan", icon: <Crown size={26} /> },
    { name: "FLASH", desc: "Promo cepat", icon: <Flame size={26} /> },
  ];
        ...
        <div className="flex items-center gap-4">
          {modes.map((item, index) => (
            <button
              key={index}
              onClick={() => {
                console.log("🔍 Shopping mode selected:", item.name);
